<template>
  <div v-if="show" class="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" @click.self="$emit('close')">
    <div class="w-full max-w-2xl rounded-xl bg-white shadow-2xl dark:bg-bg-dark-2 overflow-hidden flex flex-col max-h-[90vh]">
      <!-- Header -->
      <div class="px-6 py-6 md:px-8 flex justify-between items-center bg-white dark:bg-bg-dark-2 shrink-0">
        <div>
      ...
      <!-- Body -->
      <div class="px-6 md:px-8 pb-6 flex-1 overflow-y-auto custom-scrollbar">
      ...
      <!-- Footer -->
      <div class="px-6 py-6 md:px-8 border-t border-stroke dark:border-strokedark flex gap-3 bg-white dark:bg-bg-dark-2 shrink-0">
        <button
          @click="$emit('close')"
